...
import { NUMBER_FORMATS } from '../types/settings';
...
export default function NumberFormatModal({ 
  ...
  const getExample = (code: number) => {
    switch (code) {
      case 1: return '1,000,000';
      case 2: return '10,00,000';
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      console.error('Failed to update number format:', error);
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div className="p-6">
          <p className="text-gray-600 mb-6">Choose how large numbers should be formatted:</p>
          
          <div className="space-y-3">
            {Object.entries(NUMBER_FORMATS).map(([code, label]) => (
              <button
                key={code}
